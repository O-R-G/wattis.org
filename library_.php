...
1 ORDER BY objects.rank;";
    ...
    $thisid = $ids[0];
    $sql = "SELECT objects.id, objects.name1 FROM objects, wires WHERE wires.fromid = $thisid AND objects.active = 1 
AND wires.toid=objects.id AND wires.active = 1 ORDER BY objects.rank;";         
    $result = MYSQL_QUERY($sql);
    while ($myrow = MYSQL_FETCH_ARRAY($result))
        $submenu .= "<a href='library_.php?id=" . $myrow['id'] . "'>" . $myrow['name1'] ."</a><br/>";
